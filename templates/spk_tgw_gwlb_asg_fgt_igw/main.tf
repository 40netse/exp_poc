...
locals {
  ...
  step_num           = local.base_netmask > 24 ? 0 : local.base_netmask <= 19 ? 1 : 16
  subnet_netmask     = local.base_netmask > 24 ? 0 : local.step_num == 1 ? 24 : 28
  ...
  create_subnets = var.subnets != {} ? var.subnets : local.base_netmask > 24 ? {} : merge([for az_i in range(length(var.availability_zones)) : merge([
  ...
  # Subnets
  subnets = merge(var.existing_subnets, module.security-vpc.subnets)
...
resource "null_resource" "validation_check_subnet_cidr" {
  count = local.subnet_cidr_block != "" && local.base_netmask > 24 ? (
    <<EOT
